...
with tgt as
(select *,{{ dbt_utils.surrogate_key(check_cols)}} as cols_key from
 {{ databasemacro(DEV)}}.DBT_HSuresh.incremental_logicc),
src as
...
end as Customer_key ,src.*, {{ audit_column() }} from src left join tgt on src.customerid=tgt.customerid
